...
if __name__ == "__main__":
    ...
    def float_array_test(array_a, array_b, eps=1e-6):
        diff = array_a - array_b
        dist = np.linalg.norm(diff, axis=1)
        return np.all(dist < eps)

    def int_array_test(array_a, array_b):
        diff = array_a - array_b
        return np.all(diff == 0)

    def compare_mesh(mesh):
        success = True
        if not float_array_test(vertices, np.asarray(mesh.vertices)):
            success = False
            print('[WARNING] vertices are not the same')
        if not float_array_test(vertex_normals, np.asarray(
                mesh.vertex_normals)):
            success = False
            print('[WARNING] vertex_normals are not the same')
        if not float_array_test(
                vertex_colors, np.asarray(mesh.vertex_colors), eps=1e-2):
            success = False
            print('[WARNING] vertex_colors are not the same')
        if not int_array_test(triangles, np.asarray(mesh.triangles)):
            success = False
