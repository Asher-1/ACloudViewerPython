...
if cfg.nn_method == "sklearn":
    from sklearn.neighbors import KDTree
elif cfg.nn_method == "cloudViewer":
    from ..help_utils.cloudviewer_utils import KDTree
else:
    assert False, "unsupported nn method, please check and try again!"
...
class DataSetSemantic3D:
    ...
    def set_input_clouds(self, data_list):
        ...
        for pc in data_list:
            ...
            if self.logger is not None:
                self.logger.info("start KDTree construction (leaf_size : {})".format(self.leaf_size))

            start = time.time()
            search_tree = KDTree(sub_xyz, leaf_size=self.leaf_size)
            self.input_trees[self.cloud_split] += [search_tree]
            if self.logger is not None:
                self.logger.info("KDTree construction time cost : {} s".format(time.time() - start))
            ...
            if self.logger is not None:
                self.logger.info("KDTree projection time cost : {} s".format(time.time() - start))
    ...
    def get_tf_mapping(self):
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            ...
            for i in range(self.config.num_layers):
                if DP.pool is None:
                    self.config.nn_method = "cloudViewer"

                if self.config.nn_method == "sklearn":
                    neigh_idx = tf.py_func(DP.sklearn_knn_search, [batch_xyz, batch_xyz, self.config.k_n], tf.int32)
                elif self.config.nn_method == "cloudViewer":
                    neigh_idx = tf.py_func(DP.cloudviewer_knn_search, [batch_xyz, batch_xyz, self.config.k_n], tf.int32)
                ...
                if self.config.nn_method == "sklearn":
                    ...
                    up_i = tf.py_func(DP.cloudviewer_knn_search, [sub_points, batch_xyz, 1], tf.int32)
                elif self.config.nn_method == "cppWrapper":
