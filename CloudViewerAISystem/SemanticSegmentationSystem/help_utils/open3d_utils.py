...
class Utility:
    @staticmethod
    def get_unique_label_indices(array):
        unique_labels, indices, counts = np.unique(array, return_inverse=True, return_counts=True)
        unique_label_indices = [np.where(indices == index)[0] for index in range(len(unique_labels))]
        return unique_labels, unique_label_indices

    @staticmethod
    def create_cloud_from_array(pc_xyz_rgb):
        pc = o3d.geometry.PointCloud()
        pc.points = o3d.utility.Vector3dVector(pc_xyz_rgb[:, 0:3])
        if pc_xyz_rgb.shape[1] == 3:
            if np.max(pc_xyz_rgb[:, 3:6]) > 1:  # 0-255
                pc.colors = o3d.utility.Vector3dVector(pc_xyz_rgb[:, 3:6] / 255.)
            else:
                pc.colors = o3d.utility.Vector3dVector(pc_xyz_rgb[:, 3:6])
        return pc

    @staticmethod
    def get_clouds_by_indices(cloud, indices_array):
        return [cloud[indices] for indices in indices_array]

    @staticmethod
    def voxel_sampling(points, features=None, grid_size=0.1):
        cloud = o3d.geometry.PointCloud()
        cloud.points = o3d.utility.Vector3dVector(points)
        cv_cloud = cv3d.geometry.ccPointCloud()
        cv_cloud.set_points(cv3d.utility.Vector3dVector(points))
        if features is not None:
            if np.max(features) > 1:
                cloud.colors = o3d.utility.Vector3dVector(features / 255.)
            else:
                cloud.colors = o3d.utility.Vector3dVector(features)

            cv_cloud.set_colors(cloud.colors)
        grid_size = cv_cloud.compute_resolution()
        down_cloud = cloud.voxel_down_sample(grid_size)
        if features is not None:
            return np.asarray(down_cloud.points), np.asarray(down_cloud.colors)
        else:
            return np.asarray(down_cloud.points)
...
class IO:
    @staticmethod
    def read_image(file):
        img = o3d.io.read_image(file)
        return np.asarray(img)

    @staticmethod
    def read_mesh(file):
        mesh = o3d.io.read_triangle_mesh(file)
        pc_xyz = np.asarray(mesh.vertices)
        pc_colors = np.asarray(mesh.vertex_colors)
        pc_triangles = np.asarray(mesh.triangles)
        if len(pc_colors) == 0:
...
class KNNNeighbors:
    ...
    def multiply_knn_search(support_pts, query_pts, k):
        ...
        def task(data):
            ...
            kd_tree = o3d.geometry.KDTreeFlann(np.swapaxes(batch_data, 0, 1))
    ...
    def single_knn_search(support_pts, query_pts, k):
        ...
        for iter in range(batch_number):
            kd_tree = o3d.geometry.KDTreeFlann(np.swapaxes(support_pts[iter], 0, 1))
...
class Plot:
    ...
    def draw_pc(pc_xyzrgb, window_name=None):
        pc = Utility.create_cloud_from_array(pc_xyzrgb)
